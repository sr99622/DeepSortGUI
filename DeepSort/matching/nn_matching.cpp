...
DYNAMICM NearNeighborDisMetric::distance(const FEATURES &features, const std::vector<int>& targets)
{
...
void NearNeighborDisMetric::partial_fit(std::vector<TRACKER_DATA> &tid_feats, std::vector<int> &active_targets)
{
    for (TRACKER_DATA& data : tid_feats) {
        ...
        if (samples.find(track_id) != samples.end()) {//append
            ...
            if (newSize <= this->budget) {
                FEATURES newSampleFeatures(newSize, 128);
                ...
                if (oldSize < this->budget) {//original space is not enough;
                    FEATURES newSampleFeatures(this->budget, 128);
    ...
    for (std::map<int, FEATURES>::iterator i = samples.begin(); i != samples.end();) {
        bool flag = false;
...
Eigen::MatrixXf NearNeighborDisMetric::_pdist(const FEATURES &x, const FEATURES &y)
{
...
Eigen::MatrixXf NearNeighborDisMetric::_cosine_distance(const FEATURES & a, const FEATURES& b, bool data_is_normalized) {
