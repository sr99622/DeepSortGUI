...
void CountPanel::setNames() {
    ...
    std::cout << "CountPanel::setNames" << std::endl;
...
void CountPanel::feed(const CounterFrame& frame)
{
    if (saveOn->isChecked() && file == nullptr)
        saveOnClicked(true);

    sums.clear();
    for (const bbox_t detection : frame.detections) {
    ...
    for (const bbox_t detection : frame.detections) {
        int row = rowOf(detection.obj_id);
        if (row > -1) {
            ObjDrawer *objDrawer = (ObjDrawer*)table->cellWidget(row, 2);
            if (objDrawer->chkShow->isChecked()) {
                cv::Rect rect(detection.x, detection.y, detection.w, detection.h);
                cv::Scalar color(objDrawer->color.blue(), objDrawer->color.green(), objDrawer->color.red());
...
{
    QColor result = QColorDialog::getColor(color, mainWindow, "playqt");
    if (result.isValid()) {
        color = result;
        btnColor->setStyleSheet(getButtonStyle());
        MW->settings->setValue(getSettingsKey(), saveState());
