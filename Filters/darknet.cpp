...
Darknet::Darknet(QMainWindow *parent, const QString& baseKey)
{
    mainWindow = parent;
    name = "Darknet";
    ...
    layout->addWidget(sldrThreshold,        5, 1, 1, 3);
    layout->addWidget(lblThreshold,         5, 4, 1, 1);
    layout->addWidget(loadModel,            5, 5, 1, 1);
...
void Darknet::filter(Frame *vp)
{
    if (model == nullptr) {
        loading = true;
        model = new DarknetModel(mainWindow, this);
...
QString Darknet::cfgKey() const
{
    return baseKey + "/cfg";
}

QString Darknet::weightsKey() const
{
    return baseKey + "/weights";
}

QString Darknet::namesKey() const
{
    return baseKey + "/names";
}

QString Darknet::thresholdKey() const
{
...
void Darknet::loaderCallback(int arg)
{
    loading = false;
}
...
DarknetModel::DarknetModel(QMainWindow *parent, QObject *darknet) : QObject(parent)
{
    mainWindow = parent;
